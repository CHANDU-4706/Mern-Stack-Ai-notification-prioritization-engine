...
async function runTests() {
    ...
    const getClassification = async (eventId: string) => {
        // Wait 2 seconds for processing
        await delay(2000);
        const res = await axios.get(`${API_URL}/audit-logs`);
        const logs = res.data.data;
        const log = logs.find((l: any) => l.event_id === eventId);
        return log;
    };
    ...
    try {
        ...
        let res1 = await axios.post(`${API_URL}/events`, {
            user_id: userId,
            event_type: "SECURITY",
        ...
        const marketingRule = rules.find((r: any) => r.condition.includes("MARKETING"));

        if (marketingRule && marketingRule.is_active) {
            const resRule = await axios.post(`${API_URL}/events`, {
                user_id: userId,
                event_type: "PROMO",
